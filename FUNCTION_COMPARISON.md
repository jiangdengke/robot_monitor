...
| 前后端端到端功能验证 | `[-]` | 登录、菜单、系统、配置、航班、餐饮、缓存、代码生成、餐桌写入和数字孪生动作核心 API 烟测已通过；浏览器逐页打开仍待补充验收 |
...
| 全功能保证 | `[-]` | 真实登录、菜单和核心业务接口 API 烟测已通过；还没有完成浏览器逐页操作和真实硬件链路验证 |
...
- 剩余差距主要是浏览器逐页端到端操作验收、真实硬件/视觉/语音/AI/Kafka 外部链路接入，以及无法从部署产物还原的原始仓库逐行源码。
