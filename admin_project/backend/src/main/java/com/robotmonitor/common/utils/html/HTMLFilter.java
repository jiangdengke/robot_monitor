 ...
 */
package com.robotmonitor.common.utils.html;
...
public final class HTMLFilter {
    ...
    private String processTag(String s) {
        ...
        if (m.find() && this.allowed(name = m.group(1).toLowerCase(Locale.ENGLISH)) && !HTMLFilter.inArray(name, this.vSelfClosingTags) && this.vTagCounts.containsKey(name)) {
            this.vTagCounts.put(name, this.vTagCounts.get(name) - 1);
            return "</" + name + ">";
        }
        m = P_START_TAG.matcher(s);
        if (m.find()) {
            name = m.group(1).toLowerCase(Locale.ENGLISH);
            ...
            if (this.allowed(name)) {
                ...
                for (int ii = 0; ii < paramNames.size(); ++ii) {
                    String paramName = ((String)paramNames.get(ii)).toLowerCase(Locale.ENGLISH);
