    ...
    >
      ...
      <el-table-column v-for="column in columns" :key="column.prop" :label="column.label" :min-width="column.minWidth" :width="column.width">
        <template #default="{ row }">
          <el-tag v-if="column.tag" :type="resolveTagType(column, row)">{{ displayValue(row, column) }}</el-tag>
          <span v-else>{{ displayValue(row, column) }}</span>
        </template>
      </el-table-column>
      <el-table-column v-if="enableEdit || enableDelete || showDetail" label="操作" fixed="right" width="210">
        <template #default="{ row }">
...
async function openEdit(row) {
  ...
  try {
    ...
    resetObject(form, { ...props.defaults, ...(response.data || row) })
    formVisible.value = true
  } catch (error) {
...
async function submitForm() {
  try {
    if (formMode.value === 'create') {
      ...
      await createResource(path, form, props.createMethod)
      successMessage.value = '新增成功'
    } else {
      const path = props.updatePath || props.basePath
      await updateResource(path, form, props.updateMethod)
